...
## 12.7 สรุปท้ายบทเรียน

**แนวคิดระบบยืนยันตัวตนและการจัดการสิทธิ์:** ระบบความปลอดภัยของแอปพลิเคชันแบ่งเป็น 2 ส่วนหลักคือ การยืนยันตัวตน (Authentication) เพื่อระบุตัวตนและตรวจสอบว่าผู้ใช้คือใครจริงหรือไม่ และการอนุญาตสิทธิ์ (Authorization) เพื่อตรวจสอบสิทธิ์การเข้าถึงข้อมูลตามขอบเขตที่รับรองไว้ ซึ่งในระบบสมาร์ทโฟนไคลเอนต์จะได้รับ Access Token และ Refresh Token จากระบบคลาวด์เพื่อใช้แนบไปกับ API Request ในการยืนยันตัวตนเสมอ

**สถาปัตยกรรมระบบของ Supabase Auth:** Supabase Auth ทำงานโดยยึดรากฐานหลักผ่านระบบหลังบ้าน GoTrue API ซึ่งเป็นซอฟต์แวร์บริการที่เปิดโอกาสให้ผู้พัฒนาจัดการข้อมูลสมาชิกและสิทธิ์การเข้าถึงได้อย่างเป็นระบบ โดยรองรับทั้งรูปแบบ Magic Link, การล็อกอินผ่านโซเชียลมีเดียชื่อดัง (OAuth) หรือระบบดั้งเดิมอย่างการล็อกอินแบบใช้อีเมลและรหัสผ่าน (Email/Password Auth) ที่เป็นมาตรฐานสากล

**การสร้างฟังก์ชันสมัครสมาชิกและเข้าสู่ระบบ:** การเริ่มเปิดระบบบัญชีสมาชิกใช้เมธอด supabase.auth.signUp() โดยการส่งตัวแปรข้อมูลของอีเมลและรหัสผ่านเพื่อบันทึกข้อมูลสมาชิกใหม่ขึ้นตารางเก็บข้อมูลหลังบ้าน และใช้เมธอด supabase.auth.signInWithPassword() เพื่อสแกนตรวจสอบความถูกต้องและกู้คืนสิทธิ์เซสชันของข้อมูลสมาชิกนั้นกลับมาแสดงผลที่หน้าจอแอปพลิเคชัน

**การจัดการเซสชันผู้ใช้และตัวดักจับสถานะ:** เพื่อให้ตัวแอปพลิเคชันตื่นตัวและเปลี่ยนหน้าต่างอินเทอร์เฟซได้อย่างไหลลื่นตามสถานะผู้ใช้ปัจจุบัน เราจะใช้ตัวดักฟังความเคลื่อนไหวเซสชันของ Supabase ที่ชื่อว่า onAuthStateChange() ในการคอยฟลักซ์สัญญาณส่งข้อมูลกลับมา (Callback) เมื่อตรวจพบการสลับหน้า ข้ามเซสชัน หรือล็อกเอาต์ระบบ เพื่อควบคุมสถานะตัวแปรและวาดหน้าจอใหม่ได้อย่างถูกต้อง

**การรักษาความปลอดภัยระดับอุปกรณ์ด้วย expo-secure-store:** การเก็บบันทึกข้อมูลโทเค็นความปลอดภัยลงใน AsyncStorage มีความเสี่ยงเนื่องจากไม่มีการเข้ารหัสข้อมูล (Plaintext) ผู้พัฒนาโปรเจกต์เชิงพาณิชย์จึงบังคับใช้ expo-secure-store ในการเข้ารหัสข้อมูลระดับลึกผ่านระบบฮาร์ดแวร์ Key Store ของระบบปฏิบัติการ โดยเราสามารถจัดทำ ExpoSecureStoreAdapter มาต่อเชื่อมกับตัวแอป Supabase client ได้เพื่อความปลอดภัยขั้นสูงสุด

**การรีแฟกเตอร์ไปใช้ useContext และ AuthContext:** เมื่อโครงสร้างแอปพลิเคชันใหญ่ขึ้น การส่งผ่านค่าเซสชันข้อมูลลงไปทีละหน้าจอ (Prop Drilling) จะยุ่งยากและไร้ประสิทธิภาพ การรีแฟกเตอร์โครงสร้างสเตตล็อกอินหลักด้วยเครื่องมือ React Context API อย่างเช่น useContext และ AuthProvider จะช่วยแก้ปัญหาโดยทำหน้าที่กระจายสัญญาณส่งมอบข้อมูลสิทธิ์ไปหาคอมโพเนนต์ย่อยทั่วทั้งแอปพลิเคชันได้โดยตรง
